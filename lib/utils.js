(function initUtils(globalScope) {
  ...
  function extractListingIdFromUrl(url) {
    ...
    try {
      ...
      const paramId = parsed.searchParams.get("id") || parsed.searchParams.get("ID");
      if (paramId) return paramId;
      // mobile.de pretty URL: /auto/Make-Model-ID-12345678.html
      const pathMatch = parsed.pathname.match(/[/-]ID-(\d{6,})/i);
      return pathMatch ? pathMatch[1] : null;
    } catch (error) {
      const match = String(url).match(/[?&](?:id|ID)=([^&#]+)|[/-]ID-(\d{6,})/i);
  ...
  function makeAbsoluteUrl(url, baseUrl) {
    ...
    try {
      const fallbackBase = baseUrl || globalScope.location?.href || "https://www.avto.net";
