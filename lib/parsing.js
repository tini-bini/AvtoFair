(function initParsing(globalScope) {
  ...
  function findPrimaryImage(doc) {
    const baseUrl = doc?.location?.href || globalScope.location?.href || "https://www.avto.net";
    const ogImage = doc.querySelector("meta[property='og:image']");
    if (ogImage?.content) {
      return Utils.makeAbsoluteUrl(ogImage.content, baseUrl);
    }
    ...
    return image ? Utils.makeAbsoluteUrl(image, baseUrl) : null;
  }
  ...
  function parseListingDocument(doc, sourceUrl) {
    ...
    if (!isAvtoNetPage(url) && !isMobileDe) {
      return {
        ...
        summary: t("unsupportedListingSummary")
      };
    }
    ...
    if (!isListingPage(url, doc)) {
      return {
        ...
        summary: isMobileDe
          ? t("unsupportedPageSummaryMobile")
          : t("unsupportedPageSummaryAvto")
      };
    }
    ...
    if (!available) {
      return {
        ...
        summary: t("removedListingSummary")
      };
    }
