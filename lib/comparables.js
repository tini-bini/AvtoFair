(function initComparables(globalScope) {
  ...
  function parseComparableFromContainer(anchor, subject, sourceUrl) {
    ...
    const imageUrl = Utils.makeAbsoluteUrl(
    ...
    const comparable = {
      listingId,
      url: Utils.makeAbsoluteUrl(anchor.getAttribute("href") || anchor.href, sourceUrl),
      title: title || null,
      price,
  ...
  function parseResultsHtml(html, sourceUrl, subject) {
    ...
    const comparables = anchors
      .map((anchor) => parseComparableFromContainer(anchor, subject, sourceUrl))
      .filter(Boolean)
