(function initComparables(globalScope) {
  ...
  function parseComparableFromContainer(anchor, subject) {
    ...
    const fuel = Parsing.normalizeFuel(normalizedText.match(/(?:gorivo\s*)?(diesel|bencin|benzin|elektrika|electric|hybrid|hibrid)/i)?.[1]);
    const transmission = Parsing.normalizeTransmission(normalizedText.match(/(rocni(?:\s+\d+\s*pr\.)?|avtomatik|rocni menjalnik|samodejni menjalnik)/i)?.[1]);
